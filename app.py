...
def analyse():
    if request.method == 'POST':
        ...
        if user_input == "hi":
            chat_history.append([user_input, "Hi , How are you this is Ayurvedic Related Chat-Bot assistance"])
        # Get response from Gemini AI model
        else:
            gemini_response = chat.send_message(f" '{user_input}' for this query you need to act like a ayurvedic chatbot assistance and provide the response in short (note: provide in html format content )")
    ...
    if request.method == 'POST':
        try:
            ...
            if predd == 0:
                ...
                dho="Vata"
            prmpt =f"{dho} is my dhosa dominant prediciton  so for this u need to recommend following things  1) pathya [diet] recommendation mention brand names and ingredients 2) herbal remidies and how to take it  (give exact html format response)"
            ...
            (give exact HTML format response) short content
